...
abstract class ToolBoxFactory[U <: JavaUniverse](val u: U) {
  ...
  def mkToolBox(frontEnd: FrontEnd = mkSilentFrontEnd(), options: String = ""): ToolBox[U] =
    new { val factorySelf: this.type = this } with ToolBoxImpl[U](frontEnd, options) { toolBoxSelf =>
      type GlobalType[JU <: JavaUniverse] = ToolBoxGlobal[JU]
      protected def getGlobal(settings: scala.tools.nsc.Settings, reporter: Reporter): GlobalType[U] =
        new ToolBoxGlobal[U](settings, reporter, toolBoxSelf)
    }

  def mkTestBox(): ToolBox[U] = {
    class TestBoxGlobal[U <: JavaUniverse](settings: scala.tools.nsc.Settings, reporter0: Reporter, impl: ToolBoxImpl[U])
      extends ToolBoxGlobal[U](settings, reporter0, impl) {
      override def verifyNotTyped(expr: Tree): Unit = ()
    }
    new { val factorySelf: this.type = this } with ToolBoxImpl[U](mkSilentFrontEnd(), "") { toolBoxSelf =>
      type GlobalType[JU <: JavaUniverse] = TestBoxGlobal[JU]
      protected def getGlobal(settings: scala.tools.nsc.Settings, reporter: Reporter): GlobalType[U] =
        new TestBoxGlobal[U](settings, reporter, toolBoxSelf)
    }
  }
}

private abstract class ToolBoxImpl[U <: JavaUniverse](val frontEnd: FrontEnd, val options: String) extends ToolBox[U] { toolBoxSelf =>

  val factorySelf: ToolBoxFactory[U]
  ...
  protected def getCompilerApi: CompilerApi = new CompilerApi {
    lazy val compiler: GlobalType[U] = {
      try {
        ...
        val instance = getGlobal(command.settings, frontEndToReporter(frontEnd, command.settings))
  ...
  protected val toolBoxLock = new Object

  def withCompilerApi[T](f: CompilerApi => T): T = toolBoxLock.synchronized {
    val api = getCompilerApi
    try f(api)
...
class ToolBoxGlobal[U <: JavaUniverse](settings: scala.tools.nsc.Settings, reporter0: Reporter, impl: ToolBoxImpl[U])
  extends ReflectGlobal(settings, reporter0, impl.classLoader) {
  import impl._
  ...
  def verifyNotTyped(expr: Tree): Unit = {
  ...
  def verifyUnresolvedFreeVars(expr: Tree): Unit =
