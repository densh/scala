...
trait Reifiers { self: Quasiquotes =>
  ...
  abstract class Reifier(isReifyingExpressions: Boolean) extends {
    ...
    override def reifyName(name: Name): Tree = name match {
      case Placeholder(hole: ApplyHole) =>
        ...
        hole.tree
      case Placeholder(hole: UnapplyHole) => hole.treeAsName
    ...
    override def reifyModifiers(m: Modifiers) =
      if (m == NoMods) super.reifyModifiers(m)
        ...
        val (mods, flags) = modsPlaceholders.map {
          case ModsPlaceholder(hole: ApplyHole) => (hole.tree, hole.tpe)
        ...
        val mods = m.annotations.collect { case ModsPlaceholder(Hole(tree, _)) => tree }
        mods match {
          case tree :: Nil =>
