...
trait Holes { self: Quasiquotes =>
  ...
  class ApplyHole(card: Cardinality, splicee: Tree) extends Hole {
    ...
    val cardinality = tpe match {
      case IterableType(card, _) => card
      case _                     => NoDot
    }
  ...
  class UnapplyHole(val cardinality: Cardinality, pat: Tree) extends Hole {
    ...
    def treeAsName = fallback
    def treeAsMods = fallback
  }
  ...
  object NativeType {
    def unapply(hole: ApplyHole): Boolean = unapply(hole.tpe)
    def unapply(tpe: Type): Boolean = isNativeType(tpe)
  }
  object LiftableType {
    def unapply(hole: ApplyHole): Boolean = unapply(hole.tpe)
    def unapply(tpe: Type): Boolean = isLiftableType(tpe)
  }
  object TreeType {
    def unapply(hole: ApplyHole): Boolean = unapply(hole.tpe)
    def unapply(tpe: Type): Boolean = tpe <:< treeType
  }
  object IterableType {
    def unapply(hole: ApplyHole): Option[(Cardinality, Type)] = unapply(hole.tpe)
