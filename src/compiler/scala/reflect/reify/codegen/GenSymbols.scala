...
trait GenSymbols {
  ...
  def reifyFreeTerm(binding: Tree): Tree =
    reifyIntoSymtab(binding.symbol) { sym =>
      ...
      if (sym.isCapturedVariable) {
        ...
        Reification(name, capturedBinding, mirrorBuildCall(nme.newFreeTerm, reify(sym.name.toString), capturedBinding, mirrorBuildCall(nme.FlagsBits, reify(sym.flags)), reify(origin(sym))))
      } else {
        Reification(name, binding, mirrorBuildCall(nme.newFreeTerm, reify(sym.name.toString), binding, mirrorBuildCall(nme.FlagsBits, reify(sym.flags)), reify(origin(sym))))
      }
    }
  ...
  def reifyFreeType(binding: Tree): Tree =
    reifyIntoSymtab(binding.symbol) { sym =>
      ...
      Reification(name, binding, mirrorBuildCall(nme.newFreeType, reify(sym.name.toString), mirrorBuildCall(nme.FlagsBits, reify(sym.flags)), reify(origin(sym))))
    }
  ...
  def reifySymDef(sym: Symbol): Tree =
    reifyIntoSymtab(sym) { sym =>
      ...
      Reification(name, Ident(sym), mirrorBuildCall(nme.newNestedSymbol, reifiedOwner, reify(sym.name), reify(sym.pos), mirrorBuildCall(nme.FlagsBits, reify(sym.flags)), reify(sym.isClass)))
    }
